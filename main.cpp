...
void colorDetector(Mat img)
{
    ...
    //cout << "--------------------imgF.at(1,1)----------------------" << imgF.at(1,1) << endl;

    cout << "--------------------------------------channelsofimgF : " << imgF.channels() << ", rowsofimgF : " << imgF.rows << ", colsofimgF : " << imgF.cols<< endl;

    ofstream imgAfterFilter("/Users/lan/Desktop/TarReg/ColorDetect/imgAfterFilter.txt"); //imgAfterFilter,ios_base::out

//    if(imgAfterFilter.is_open())
//    {
//        imgAfterFilter << "this is testing line." << endl;
//        imgAfterFilter << imgF << endl;
//        cout << "---------------------------------------------------------------------------------------" << endl;
//        imgAfterFilter.close();
//    }
...
//        {
//            uchar* pixelPtr = imgF.ptr<uchar>(r);
//            for (int c = 0; c < imgF.cols*imgF.channels(); c++)
//            {
//                int data = pixelPtr[c];  //读取数据，at<type> - type 是矩阵元素的具体数据格式
//                imgAfterFilter << data << " " ;   //每列数据用 tab 隔开
//            }
//            imgAfterFilter << endl;  //换行
//        }
//    }

    //imgAfterFilter << img << endl;
    ...
    cout << "----------------channelsofimgHSV : " << imgHSV.channels() << ", rowsofimgHSV : " << imgHSV.rows << ", colsofimgHSV : " << imgHSV.cols << endl;

    ofstream imgF_AfterHSV("imgF_AfterHSV.txt",ios_base::out);
    imgF_AfterHSV << imgF << endl;
    imgF_AfterHSV.close();
    ...
    cout << "-----------------------------------------channelsofimgBlue : " << imgBlue.channels() << ", rowsofimgBlue : " << imgBlue.rows << ", colsofimgBlue : " << imgBlue.cols << endl;
    //cout << "imgBlue Mat = " << imgBlue << endl;
    ofstream imgBlueAfterinRange("imgBlueAfterinRange.txt",ios_base::out);
    imgBlueAfterinRange << imgBlue << endl;
    imgBlueAfterinRange.close();
